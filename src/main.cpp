...
int main(int argc, char *argv[])
{
    ...
    // Prefer explicit qrc alias first (most reliable on Android)
    {
        ...
        for (const QString &path : paths) {
            if (tryLoadData(engine, path)) {
                loaded = true;
                qWarning() << "GeoField UI loaded via loadData" << path;
                break;
            }
        }
    }
    ...
    if (!loaded) {
        ...
        for (const QUrl &u : urls) {
            if (tryLoadUrl(engine, u)) {
                loaded = true;
                qWarning() << "GeoField UI loaded via URL" << u;
                break;
            }
        }
    }
    ...
    if (!loaded) {
        ...
        if (loaded)
            qWarning() << "GeoField UI loaded via module";
    }
