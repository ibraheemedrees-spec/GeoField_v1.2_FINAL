...
static QStringList g_qmlErrorLog;

static void appendQmlErrors(const QList<QQmlError> &errs)
{
...
static QString collectedQmlErrors()
{
    if (g_qmlErrorLog.isEmpty())
        return QStringLiteral("(no QQmlError captured — check logcat for QML: lines)");
    return g_qmlErrorLog.join(QLatin1Char('\n'));
}

static QObject *g_createdRoot = nullptr;

static bool loadMainUi(QQmlApplicationEngine &engine, QString *errorOut)
{
    g_qmlErrorLog.clear();
    g_createdRoot = nullptr;

    QObject::connect(&engine, &QQmlApplicationEngine::warnings,
                     &engine, [](const QList<QQmlError> &w) { appendQmlErrors(w); });
    QObject::connect(&engine, &QQmlApplicationEngine::objectCreationFailed,
                     &engine, [](const QUrl &url) {
                         const QString line = QStringLiteral("objectCreationFailed: %1").arg(url.toString());
                         qCritical() << "QML ERROR:" << line;
                         g_qmlErrorLog << line;
                     });

    const QByteArray data(kEmbeddedMainQml); // copy — safer than fromRawData for long-lived use
    qWarning() << "UI: embedded QML bytes" << data.size();

    // 1) Preferred: loadData registers rootObjects on QQmlApplicationEngine
    engine.loadData(data, QUrl(QStringLiteral("qrc:/main.qml")));
    if (!engine.rootObjects().isEmpty()) {
        qWarning() << "UI: loadData OK roots=" << engine.rootObjects().size();
        return true;
    }
    appendQmlErrors(QList<QQmlError>()); // keep log from warnings signal
    qWarning() << "UI: loadData produced no roots; trying QQmlComponent";

    // 2) QQmlComponent — precise errors + manual create
    QQmlComponent component(&engine);
    component.setData(data, QUrl(QStringLiteral("qrc:/main.qml")));
    if (component.isError()) {
        appendQmlErrors(component.errors());
        qCritical() << "UI: QQmlComponent compile/parse error";
    } else {
        ...
        if (obj) {
            ...
            g_createdRoot = obj;
            if (obj->property("visible").isValid())
                obj->setProperty("visible", true);
            qWarning() << "UI: QQmlComponent create OK" << obj->metaObject()->className();
            return true; // SUCCESS even if rootObjects() empty
        }
        appendQmlErrors(component.errors());
        qCritical() << "UI: QQmlComponent create returned null";
    }

    // 3) Module from qt_add_qml_module
    engine.loadFromModule(QStringLiteral("GeoField"), QStringLiteral("main"));
    if (!engine.rootObjects().isEmpty()) {
        qWarning() << "UI: module OK";
        return true;
    }

    if (errorOut)
        *errorOut = collectedQmlErrors();
    qCritical() << "UI LOAD FAILED:\n" << collectedQmlErrors();
    return false;
}
...
int main(int argc, char *argv[])
{
    ...
    QString loadErrors;
    const bool uiOk = loadMainUi(engine, &loadErrors);
    if (!uiOk && !g_createdRoot) {
...
Window {
    ...
    if (engine.rootObjects().isEmpty() && !g_createdRoot) {
