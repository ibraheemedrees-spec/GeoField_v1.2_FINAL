...
// Compact professional UI embedded in binary — does not depend on APK qrc packaging.
...
static bool loadMainUi(QQmlApplicationEngine &engine)
{
    ...
    // Primary: embedded QML (always in the binary)
    {
        ...
        qWarning() << "UI: embedded load failed";
    }

    // Secondary: qrc / module
...
int main(int argc, char *argv[])
{
    ...
    if (!loadMainUi(engine)) {
        qCritical() << "UI: all load methods failed";
        static const char *kFallback = R"QML(
import QtQuick
import QtQuick.Window
Window {
    visible: true; width: 400; height: 700; color: "#101820"
    Text {
        anchors.centerIn: parent
        text: "UI Load Failed"
        color: "#ff5252"; font.pixelSize: 20; font.bold: true
    }
}
)QML";
