...
static bool loadMainUi(QQmlApplicationEngine &engine, QString *errorOut)
{
    ...
    // Use QQmlComponent to get precise errors() on failure
    ...
    if (component.isError()) {
        ...
        if (obj) {
            // loadData path equivalent: keep object alive as child of engine
            engine.setObjectOwnership(obj, QQmlEngine::JavaScriptOwnership);
            qWarning() << "UI: EMBEDDED main created bytes" << data.size();
            // Track as root for emptiness check — store on engine via property
            obj->setParent(engine.rootContext()->engine());
            // QQmlApplicationEngine::rootObjects won't include this; force success path:
            // Fall through to loadData which registers roots properly.
        }
        appendQmlErrors(component.errors());
    }

    // Reliable root registration
    if (engine.rootObjects().isEmpty()) {
        engine.loadData(data, QUrl(QStringLiteral("qrc:/main.qml")));
    }
    if (!engine.rootObjects().isEmpty()) {
        qWarning() << "UI: EMBEDDED main loaded (rootObjects)" << engine.rootObjects().size();
        return true;
    }
